		...
		if (Context.defined("android"))
		{
			func.expr = macro
			{
				if ($i{fieldName} == null)
				{
					...
					if ($v{isStaticMethod})
					{
						$i{fieldName} = #if (openfl && (openfl < "4.0.0")) openfl.utils#else lime.system#end.JNI.createStaticMethod(
							$v{jniPackage}, $v{jniPrimitive}, $v{jniSignature});
					}
					else
					{
						$i{fieldName} = #if (openfl && (openfl < "4.0.0")) openfl.utils#else lime.system#end.JNI.createMemberMethod(
							$v{jniPackage}, $v{jniPrimitive}, $v{jniSignature});
					}
					#end
				}
